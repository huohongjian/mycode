...
{
	var editHtmlData = '';
	editHtmlData += '<html<head><title>KindEditor</title>';
	editHtmlData += '<link href="'+KE_CSS_PATH+'" rel="stylesheet" type="text/css"></head><body>';
	editHtmlData += content;
	editHtmlData += '</body></html>';
	documentObj.open();
...
function KindEditor(objName) 
{
	...
	this.show = function()
	{
		...
		if (KE_EDITFORM_DOCUMENT.addEventListener){
			...
			/*hhj add 回车换行 tab空四格*/
			el.attachEvent('onkeydown', function(e){
				if(e.keyCode==13) {
					var sel = KE_EDITFORM_DOCUMENT.selection;
					if (sel!=null) {
	...
	this.data = function()
	{
		...
		return htmlResult.replace(/\n|\r/g, '');	/*hhj add replace for ie*/
	}
	
	this.write = function(html) {
//		KindSimpleExecute('selectall');	//两种方法都可以 但ff有异步
//		KindInsertHtml(html);
		KindWriteFullHtml(KE_EDITFORM_DOCUMENT,html);
	}
}
